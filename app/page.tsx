...
export default function Home() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        // Fetch from dna.json in public folder
        const response = await fetch('/dna.json');
        if (response.ok) {
  ...
  if (hasError) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-black text-white font-mono p-4 text-center">
        <h1 className="text-4xl md:text-6xl font-bold mb-4 tracking-tighter text-red-500">NO SIGNAL</h1>
        <p className="text-sm md:text-base opacity-70">Please upload your <span className="text-white font-bold">dna.json</span> to the public folder.</p>
      </div>
    );
  }
